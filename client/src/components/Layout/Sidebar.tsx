...
useEffect(() => {
  if (!isDir && !isMgr && !isTeamLd) {
    ...
    axios
      .get(`https://nts-erp-system-629k.vercel.app/api/employee/overview`, {
...
useEffect(() => {
  ...
  setEmployeeStats(prev => ({ ...prev, leaveBalance: remaining }));
}, [remaining]);
  ...
  const fetchData = async () => {
    try {
      ...
      const res = await axios.get(
        `https://nts-erp-system-629k.vercel.app/api/manager/overview/${user?.id}`, // 👈 include userId
        {
          headers: {
