...
interface EmployeeStats {
  ...
  dailyProgressStatus?: string; // optional
}
...
const [employeeStats, setEmployeeStats] = useState<EmployeeStats | null>(null);
  ...
  // Fetch overview & leaves
useEffect(() => {
  if (!isDir && !isMgr && !isTeamLd) {
    setLoading(true);
    const token = localStorage.getItem("token");

    axios
      .get(`${baseURL}/api/employee/overview`, {
...
}, [user?.id, isDir, isMgr, isTeamLd]);

  console.log("Current logged-in user ID:", user?.id);

const currentUserLeaves = useMemo(
...
const approvedLeaves = useMemo(
  () => currentUserLeaves.filter(lr =>
    lr.status === "approved" ||
    lr.manager_approval === "approved" ||
    lr.director_approval === "approved"
  ),
  [currentUserLeaves]
);

// 3️⃣ Calculate used and remaining leave days
const { usedDays, remaining } = useMemo(() => {
  const days = approvedLeaves.reduce((sum, lr) => {
  ...
  return { usedDays: days, remaining: Math.max(0, LEAVE_QUOTA - days) };
}, [approvedLeaves]);
...
setEmployeeStats(prev => ({
  ...
  totalTasks: prev?.totalTasks ?? 0,
  completedTasks: prev?.completedTasks ?? 0,
  dailyProgressStatus: prev?.dailyProgressStatus ?? "No Update"
}));

}, [remaining]);

// // ✅ Debug logs
// console.log("Leaves for current user:", currentUserLeaves);
// console.log("Approved leaves for current user:", approvedLeaves);
// console.log("Used days:", usedDays, "Remaining days:", remaining);
...
useEffect(() => {
  const fetchData = async () => {
    try {
      ...
      const res = await axios.get(
        `${baseURL}/api/manager/overview/${user?.id}`, // 👈 include userId
        {
          headers: {
      ...
      console.log("✅ Raw Manager Overview Response:", res.data);
  ...
  if (isMgr && user) {
    console.log("🔍 Fetching overview for manager:", user.id);
    fetchData();
  }
}, [user?.id, isMgr]);
...
useEffect(() => {
  if (teamMetrics?.projects) {
    console.log("📊 All Projects from API:", teamMetrics.projects); // <-- log projects before filtering

    let projects = teamMetrics.projects;

    if (filter === "completed") {
      projects = projects.filter((p: { status: string; }) => p.status === "completed");
    } else if (filter === "pending") {
      projects = projects.filter((p: { status: string; }) => p.status === "pending");
    } else if (filter === "active") {
      projects = projects.filter((p: { status: string; }) => p.status === "active");
    }
  ...
  // 🔹 Role-based nav items (can also come from backend if needed)
  ...
  function toggleSection(arg0: string): void {
            ...
            <div className="bg-white rounded-lg p-3 border border-green-100">
              <div className="flex items-center space-x-2">
                ...
                <div>
                  ...
                  <p className="text-xs text-green-700">Completed Tasks</p>
                </div>
              </div>
            </div>
              ...
              <div className="flex items-center space-x-2">
                ...
                <div>
                  <p className="text-lg font-bold text-orange-900">
                    {teamMetrics.missingReports}
                  </p>
                  <p className="text-xs text-orange-700">Missing Reports</p>
                </div>
              </div>
            </div>
          </div>
          ...
          <div className="bg-white rounded-lg p-3 border border-gray-100">
            <div className="flex items-center justify-between mb-2">
              ...
              <span className="text-sm font-bold text-gray-900">
                {teamMetrics.avgProgress}%
              </span>
            </div>
            <div className="w-full bg-gray-200 rounded-full h-2">
              <div
                className={`h-2 rounded-full transition-all duration-500 ${
                  teamMetrics.avgProgress >= 80
                    ? 'bg-gradient-to-r from-green-400 to-green-600'
                    : teamMetrics.avgProgress >= 60
                    ? 'bg-gradient-to-r from-blue-400 to-blue-600'
                    : teamMetrics.avgProgress >= 40
